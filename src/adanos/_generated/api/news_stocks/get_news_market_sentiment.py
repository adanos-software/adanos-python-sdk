...
from ...models.news_market_sentiment_response import NewsMarketSentimentResponse
...
def _get_kwargs(
    *,
    ...
    _kwargs: dict[str, Any] = {
        "method": "get",
        "url": "/news/stocks/v1/market-sentiment",
        "params": params,
    }
...
) -> ErrorResponse | HTTPValidationError | InvalidPeriodError | HistoricalLimitError | NewsMarketSentimentResponse | RateLimitError | None:
    if response.status_code == 200:
        response_200 = NewsMarketSentimentResponse.from_dict(response.json())
...
def sync_detailed(
    *,
    ...
    """Market Sentiment
     ...
     Returns the service-level News market sentiment snapshot across all tracked stocks.

    Use this endpoint when you want a single News-wide market state instead of per-ticker rankings.
    ...
    - higher = broader / hotter coverage than usual
    - lower = quieter than usual

    `trend` is service-level news activity momentum over current 3 UTC days vs
    previous 3 UTC days using mentions/articles and source breadth. It is not price
    trend.
